"""Browser demo: autoplay stories through the real unified pipeline (stdlib only).

    python3 -m zeroguard.demo
    http://zeroguard.localhost:8873/
"""
...
from zeroguard.catalog import DEMO_AUDIT, HOST, PORT, ROOT, STATIC, STORIES, STORY_ORDER, TITLE
...
class DemoHandler(BaseHTTPRequestHandler):
    server_version = "ZeroGuardDemo/1.0"
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/stories":
            self._send_json({"order": STORY_ORDER, "blurbs": {k: v[3] for k, v in STORIES.items()}})
            return
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--no-peers", action="store_true", help="Do not start CICD/infra demos")
    args = parser.parse_args(argv)
    DEMO_AUDIT.parent.mkdir(parents=True, exist_ok=True)
    if not args.no_peers:
        started = start_peer_demos(desktop=ROOT.parent, self_port=args.port)
        if started:
            print("also started " + ", ".join(started), flush=True)
    server = ThreadingHTTPServer((HOST, args.port), DemoHandler)
    url = f"http://{HOST}:{args.port}/"
    print(f"{TITLE}  {url}  (Ctrl+C to stop)", flush=True)
