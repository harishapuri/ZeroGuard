...
from zeroguard.catalog import EXPECTED, STORIES, STORY_ORDER
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Automate ZeroGuard demo stories on the unified gate.")
    ...
    if args.story != "all" and args.story not in STORIES:
        ...
        return 2
    rows = run_all(keys)
    print(json.dumps({"plane": "zeroguard", "stories": rows, "passed": all(r["ok"] for r in rows)}, indent=2))
